const weather_codes = {
     ...
     56: {
          name: "Chuvisco Leve e Congelante",
          icons: {
               day: "drizzle.svg",
               night: "drizzle.svg"
          }
     ...
     57: {
          name: "Chuvisco Forte e Congelante",
          icons: {
               day: "heavy-drizzle.svg",
               night: "heavy-drizzle.svg"
          }
     ...
     67: {
          name: "Chuva Forte e Congelante Extr.",
          icons: {
               day: "heavy-rain.svg",
               night: "heavy-rain.svg"
          }
     ...
     80: {
          name: "Aguaceiros fracos de chuva",
          icons: {
     ...
     81: {
          name: "Pancadas moderadas de chuva",
          icons: {
     ...
     82: {
          name: "Pancadas muito fortes de chuva",
          icons: {
     ...
     96: {
          name: "Trovoada fraca com granizo",
          icons: {
               day: "hail.svg",
               night: "hail.svg"
          }
     ...
     99: {
          name: "Trovoada forte com granizo",
          icons: {
               day: "heavy-hail.svg",
...
const temperatureTxt = document.getElementById("temperatura");
...
async function getLocation(location) {
     const res = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${location}&count=1&language=pt&format=json`);
...
async function setRandomCityBackground(city) {
    try {
        const res = await fetch(
            `https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(city)}&srnamespace=6&srlimit=25&format=json&origin=*`
        );

        const data = await res.json();
        if (!data.query || !data.query.search.length) return;

        const random =
            data.query.search[Math.floor(Math.random() * data.query.search.length)];

        const imgRes = await fetch(
            `https://commons.wikimedia.org/w/api.php?action=query&titles=${encodeURIComponent(random.title)}&prop=imageinfo&iiprop=url&format=json&origin=*`
        );
        ...
        console.warn("Imagem aleatória não encontrada");
    }
}
...
changeBgBtn.addEventListener("click", () => {
    if (lastCity) {
        setRandomCityBackground(lastCity);
    }
});
...
searchBox.addEventListener("submit", async e => {
     ...
     if (locationInput.value.trim() === "") {
          errTxt.textContent = "Por favor informe uma cidade"
     } else {
          errTxt.textContent = ""
          try {
               const weather = await getWeather(locationInput.value)
               lastCity = weather.name;
               await setRandomCityBackground(lastCity);
               ...
               errTxt.textContent = "Cidade  não encontrada"
          }
     }
}) 